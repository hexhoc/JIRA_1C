...
Функция НайтиСоздатьЗадачу(ДанныеЗадачи, ПринудительнаяЗагрузка) Экспорт
		...
		ДатаИзмененияЗагружаемого = JIRA_ВзаимодействиеC_JIRA_КлиентСервер.ПреобразоватьВДату(ДанныеЗадачи["fields"]["updated"]);
...
Процедура ОбновитьДанныеЗадачи(ЗадачаОбъект, ДанныеЗадачи, ПринудительнаяЗагрузка)
	...
	ДатаСоздания 			= JIRA_ВзаимодействиеC_JIRA_КлиентСервер.ПреобразоватьВДату(ПоляЗадачи["created"]);
	ДатаИзменения 			= JIRA_ВзаимодействиеC_JIRA_КлиентСервер.ПреобразоватьВДату(ПоляЗадачи["updated"]);
	...
	ПлановаяДатаЗавершения 	= JIRA_ВзаимодействиеC_JIRA_КлиентСервер.ПреобразоватьВДату(ПоляЗадачи["customfield_11501"]);
	//	
...
Процедура ОбновитьИсториюИзмененияСтатусов(ЗадачаОбъект, ЗадачаСсылка, Лог)
	...
	Для Каждого ЗаписьЛога Из Лог["histories"] Цикл
		Для Каждого Изменение Из ЗаписьЛога["items"] Цикл
			...
			Запись.ДатаСобытия = JIRA_ВзаимодействиеC_JIRA_КлиентСервер.ПреобразоватьВДату(ЗаписьЛога["created"]);
			Запись.НомерЗаписи = Набор.Количество();
...
Процедура ВыполнитьЗагрузкуЖурналаРабот(ЗадачаОбъект, ЗадачаСсылка)
	...
	ЖурналРабот = JIRA_ВзаимодействиеC_JIRA_КлиентСервер.ПолучитьЛогСписанийПоЗадаче(ЗадачаОбъект.Код);
	...
	Для Каждого ЗаписьОРаботе из ЖурналРабот цикл
		...
		МенеджерЗаписи.ДатаСоздания 	= JIRA_ВзаимодействиеC_JIRA_КлиентСервер.ПреобразоватьВДату(ЗаписьОРаботе["started"]);
		МенеджерЗаписи.Задача 			= ЗадачаСсылка;
