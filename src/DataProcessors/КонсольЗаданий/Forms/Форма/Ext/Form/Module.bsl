...
Процедура СписокРегламентныхЗаданийПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	// {{ e.nehoroshih; 000000398789; 27.03.2018
	Диалог = ПолучитьФорму(Объект.ПутьКФормам  + ".ДиалогРегламентногоЗадания", СтруктураПараметров);
	// e.nehoroshih; 000000398789; 27.03.2018 }}
...
Процедура УдалитьРегламентноеЗадание()
	...
	Для Каждого Стр из ВыделенныеСтроки Цикл
		...
		РегламентноеЗадание = ОбъектОбработки().ПолучитьОбъектРегламентногоЗадания(Строка.Идентификатор);
...
Процедура СписокФоновыхЗаданийПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Попытка
		// {{ e.nehoroshih; 000000398789; 27.03.2018
		Диалог = ПолучитьФорму(Объект.ПутьКФормам  + ".ДиалогФоновогоЗадания", СтруктураПараметров);
		// e.nehoroshih; 000000398789; 27.03.2018 }}
		Если Диалог.ОткрытьМодально() <> Неопределено Тогда
		    ОбновитьСписокФоновыхЗаданий();			
		КонецЕсли;
...
Функция УстановитьРасписаниеРегламентногоЗадания(Идентификатор, Наименование, Расписание, ИмяЗадания)
	ОбъектЗадания = ОбъектОбработки().ПолучитьОбъектРегламентногоЗадания(Идентификатор);		
	Если ОбъектЗадания = Неопределено Тогда
...
Процедура НастройкаСпискаРегламентныхЗаданий(Команда)
	...
	// {{ e.nehoroshih; 000000398789; 27.03.2018
	Диалог = ПолучитьФорму(Объект.ПутьКФормам  + ".ДиалогНастроекСписка", СтруктураПараметров);
	// e.nehoroshih; 000000398789; 27.03.2018 }}
...
Процедура УстановитьОтборРегламентныхЗаданий(Команда)
	...
	// {{ e.nehoroshih; 000000398789; 27.03.2018
	Диалог = ПолучитьФорму(Объект.ПутьКФормам  + ".ДиалогОтбораРегламентногоЗадания", СтруктураПараметров);
	// e.nehoroshih; 000000398789; 27.03.2018 }}
	Результат = Диалог.ОткрытьМодально();
	Если Результат <> Неопределено Тогда
...
Процедура УстановитьОтборФоновыхЗаданий(Команда)
	...
	// {{ e.nehoroshih; 000000398789; 27.03.2018
	Диалог = ПолучитьФорму(Объект.ПутьКФормам  + ".ДиалогОтбораФоновогоЗадания", СтруктураПараметров);
	// e.nehoroshih; 000000398789; 27.03.2018 }}
	Результат = Диалог.ОткрытьМодально();
	Если Результат <> Неопределено Тогда
